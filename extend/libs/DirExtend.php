...
class DirExtend
{
     ...
     * @return array
     */
    public static function getJsTreeTpl(string $dir, string $filterExt = '', bool $bool = true)
    {
        $jstree = [];
        ...
        foreach ($di as $key=>$value) {
            ...
            if ($value->isDir()) {
                ...
                $jstree[$i]['children'] = static::getJsTreeTpl($value->getRealPath() . DIRECTORY_SEPARATOR, $filterExt, $bool);
            } else {
     ...
     * @return array|false
     */
    public static function rglob($pattern, $flags = 0, $ignore = [])
    {
        //获取子文件
        ...
        if (is_array($subdir)) {
            foreach ($subdir as $dir) {
                if ($ignore && in_array($dir, $ignore))
                    continue;
                $files = array_merge($files, static::rglob($dir . DIRECTORY_SEPARATOR . basename($pattern), $flags, $ignore));
            }
        }
        return $files;
    }
    ...
    public static function delDir($directory, $subdir=true)
    {
        ...
        while (($file = readdir($handle)) !== false) {
            if ($file != "." && $file != "..") {
                is_dir("$directory/$file") ? static::delDir($directory.DIRECTORY_SEPARATOR.$file) : unlink($directory.DIRECTORY_SEPARATOR.$file);
            }
        }
     ...
     * @return bool
     */
    public static function delFile($directory)
    {
        if (is_dir($directory) == false) {
            $this->error = lang('Directory does not exist');
            return false;
        ...
        while (false !== ($file = readdir($handle))) {
            if ($file != "." && $file != "..") {
                is_dir($source . DIRECTORY_SEPARATOR .$file) ?
                                static::copyDir($source . DIRECTORY_SEPARATOR .$file, $destination.DIRECTORY_SEPARATOR.$file) :
     ...
     * @return bool
     */
    public static function movedFile($tmpdir, $newdir, $pack = null)
    {
