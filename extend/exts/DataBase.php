...
class Database
{
    ...
    public function import($start)
    {
        ...
        if ($this->config['compress']) {
            ...
            $size = filesize($this->file)*7.5;
        } else {
            $size = filesize($this->file);
            $gz = fopen($this->file, 'r');
        }
        $line = count(file($this->file))+1;
        $sql = '';
    ...
    public function Backup_Init()
    {
        ...
        $sql .= "-- \n";
        $sql .= "-- Host     : " . $this->dbconfig['connections'][$this->dbconfig['default']]['hostname'] . "\n";
        $sql .= "-- Port     : " . $this->dbconfig['connections'][$this->dbconfig['default']]['hostport'] . "\n";
        $sql .= "-- Database : " . $this->dbconfig['connections'][$this->dbconfig['default']]['database'] . "\n";
        $sql .= "-- \n";
