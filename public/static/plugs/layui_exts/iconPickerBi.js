...
layui.define(['laypage', 'form'], function (exports) {
    ...
    IconPicker.prototype.render = function (options) {
        ...
        var a = {
            ...
            createSelect: function () {
                var oriIcon = '<i class="bi">';

                // 默认图标
                if (ORIGINAL_ELEM_VALUE === '') {
                    ORIGINAL_ELEM_VALUE = 'bi-heart';

                }


                oriIcon = '<i class="bi ' + ORIGINAL_ELEM_VALUE + '">';
            ...
            createBody: function () {
                ...
                if (search) {
                    searchHtml = '<div class="layui-iconpicker-search">' +
                        '<input class="layui-input" data-auto-none>' +
            ...
            check: function () {
                ...
                a.event('click', item, function (e) {
                    var el = $(e.currentTarget).find('.bi'),
                        icon = '';

                    var clsArr = el.attr('class').split(/[\s\n]/),
                        cls = clsArr[1],
                        icon = cls;
                    $('#' + TITLE_ID).find('.layui-iconpicker-item .bi').html('').attr('class', clsArr.join(' '));
            ...
            getData: function (url) {
                ...
                $.ajax({
                    ...
                    success: function (ret) {
                        var exp = /\.bi-(.*)::before/ig;
                        var result;
                        while ((result = exp.exec(ret)) != null) {
                            iconlist.push('bi-' + result[1]);
                        }
                    },
    ...
    IconPicker.prototype.checkIcon = function (filter, iconName) {
        var el = $('*[lay-filter=' + filter + ']'),
            p = el.next().find('.layui-iconpicker-item .bi'),
            c = iconName;

        if (c.indexOf('#xe') > 0) {
            p.html(c);
        } else {
            p.html('').attr('class', 'bi ' + c);
        }
