...
window.tapiRoot = window.appRoot + "adx.php";
...
$(function () {
        ...
        this.reInit = function ($dom) {
            ...
            return $dom.find('[required]').map(function () {
                ...
                this.setAttribute('autocomplete', 'off'), laydate.render({
                    type: this.dataset.dateRange || 'date', range:'~', elem: this, done: function (value) {
    ...
    /*! ICON图标选择 */
    $.fn.iconSelect = function () {

		return this.each(function () {
			if ($(this).data('inited')) return true; else $(this).data('inited', true);
			var $this = $(this);
			layui.use(['iconPickerBi'], function(){
				layui.iconPickerBi.render({
					 url: "/static/vendor/bootstrap-icons-1.8.2/bootstrap-icons.css",
					 elem: $this, search: true,page: true,limit: 15,cellWidth: '42px'
				});
			});
    ...
    onEvent('click', '[data-modal]', function () {
        ...
        return $.form.modal(emap.modal, data, emap.title, un, un, un, emap.area || emap.width || '800px', emap.offset || 'auto', emap.full !== un);
    });
