...
use app\common\library\Images as imagesLibrary;
        ...
        try {
            if ($this->getType() === 'local') {
                ...
                if (in_array($extension, ['jpg', 'gif', 'png', 'bmp', 'jpeg', 'wbmp'])) {
					...
					// 设置图片压缩
					$compressConfig = sysconf('compress.');
					if( $compressConfig['enable'] == '1' ){

						if (
							(isset($compressConfig['size']) && filesize($distName) > ($compressConfig['size'] * 1024 * 1024))
							|| ( isset($compressConfig['width']) && $width > $compressConfig['width'] ) 
							|| ( isset($compressConfig['height']) && $height > $compressConfig['height'] )
						){
							$imagesLibrary = new imagesLibrary();
							$imagesLibrary->compress($info['file'], $compressConfig);
						}
					}

					//设置图片水印
					$watermarkConfig = sysconf('watermark.');
					if( $watermarkConfig['enable'] == '1' ){
						if ( 
							( isset($watermarkConfig['minwidth']) && $width > $watermarkConfig['minwidth'] ) 
							|| ( isset($watermarkConfig['minheight']) && $height > $watermarkConfig['minheight'] )
						){
							$imagesLibrary = new imagesLibrary();
							$imagesLibrary->waterMark($info['file'],$watermarkConfig);
						}
					}
                }
